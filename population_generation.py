...
def individual(max_len: int) -> Genome:
    return array('B', [rand.randrange(256) for _ in range(rand.randint(1, max_len))])
...
def generate_next_population(population: List[Tuple[float, Genome]], max_len: int, tournament_size: int, weights: Tuple[float, float, float],
    ...
    # mutation
    new_population.extend([mutation(tournament_precomputed(population, tournament_size), max_len) for _ in range(math.floor(pop_len * weights[1]))])
    # reproduction
